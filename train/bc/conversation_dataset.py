"""TAP 预训练样本 → Unsloth 视觉 SFT 的对话格式数据集。

对外接口：
    DEFAULT_INSTRUCTION — 动作预测任务的默认指令文本。
    SubsetName — 可读取的子集名。
    build_conversation — 单条 TAP 样本 → messages 对话。
    load_tap_conversations — 读 ``samples_<子集>.jsonl``，产出可直接喂 SFTTrainer 的数据集。

对话布局遵循 Unsloth 视觉微调的硬约束：**图像必须排在文本指令之前**，assistant 回复
只含 TAP 动作串，使模型的监督目标就是动作 token 本身。
"""
...
def load_tap_conversations(
    ...
    """读取 TAP 预训练目录的一个子集，产出 SFTTrainer 可直接消费的对话列表。
    ...
    if not samples_path.is_file():
        raise FileNotFoundError(
            f"找不到 {samples_path}；先跑 dataset.organization.pretraining",
        )
