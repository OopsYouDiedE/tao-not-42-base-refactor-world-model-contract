...
from tools.trajectory_human_review import ReviewStore, format_reference_actions
...
def dataset(root: Path) -> None:
    write_jsonl(root / "questions.jsonl", [{
        "id": "q", "task_type": "image_sequence_to_action", "images": [],
        "source": {"episode": "e", "image_frames": []}, "target_interval": [0, 4],
        "prompt": "infer", "inputs": {},
    }])
    write_jsonl(root / "answer_key.jsonl", [{
...
def test_preannotation_does_not_count_as_human_review(tmp_path: Path) -> None:
    dataset(tmp_path)
    write_jsonl(tmp_path / "ai_question_preannotations.jsonl", [{
        "id": "q", "decision": "reject", "reason": "目标不清楚",
...
def test_complete_reference_actions_are_rendered() -> None:
    rendered = format_reference_actions(
        ["tick one", "tick two"], node_frames=[20, 25, 28], unoptimized=True,
    )
