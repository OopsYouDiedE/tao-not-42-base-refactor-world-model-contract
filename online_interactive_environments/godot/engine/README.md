...
- **Python 环境包 → `online_interactive_environments/godot/`**：共享内存、进程管理、SB3 适配和 PPO 训练入口。
...
设置 `GODOT_EXE` 后运行 `python -m online_interactive_environments.godot.train_ppo`。入口启动 Godot，并由 `GodotVectorizedEnvironment` 驱动 PPO。Python 与 Godot 通过文件映射通信，不使用 `godot-python`。
