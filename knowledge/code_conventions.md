...
- `net/backbone.py` 的 `MockDINOv2` 与 AGENTS §2("mock 仅许在 tests/")有张力:它是
  `--encoder mock` 离线冒烟骨干(显式 CLI 选择、非静默降级)。两条出路待定——经依赖注入移入
  `tests/`,或在 AGENTS §2 正式确认这一窄例外。当前按"显式 affordance"保留在 `net/`。
