...
## 1. 两条正交轴："生成"与"自回归"是两个独立的维度

近年模型可以沿两条独立的轴来定位，以避免概念混淆：

- **轴 A — 输出空间**：像素 / token 生成（解码回画面）vs 潜空间预测（只预测表征，不解码）。
- **轴 B — 时间推进**：自回归 rollout（把预测喂回作为下一步输入，多步想象）vs 单步 / 能量规划。

**关键说明**：本项目"不做自回归"针对的是**轴 A 的像素自回归生成**。而**轴 B 的潜空间 rollout** 是另一回事——DINO-WM、V-JEPA 2-AC、DreamerV3 都在**潜空间里自回归 rollout** 做规划，且都不解码像素。因此本项目的立场应精确表述为：**不做自回归像素 / token 生成；潜空间多步想象不在禁止之列**，它与 Δz 预测、变 Δt 兼容，且是潜空间规划的前提。
...
- **对本项目**：证实"冻结 DINO + ViT 潜预测 + decoder-free"是可行的方向。差异有二：① 我们把 patch 进一步压到 **N 个实体槽**——压缩程度更高，换来实体级可控，但存在信息瓶颈风险（对应 oracle_idm 触顶的历史观察）；② 我们（旧 Δz-JEPA 线）额外有 `c` 闸 + 逆动力学接地（原 `net/world_model.py` 的 `inv_dyn` 与 `minecraft_inv_dyn_loss`，已随旧管线退役），可视作 DINO-WM 的超集。**最大差异：DINO-WM 在推理期跑 MPC，我们当前不跑（见 §5）。**
...
## 3. 像素生成交互世界模型（对照类，开源，可参考的教训）

这一类属"轴 A 像素生成 + 轴 B 自回归"，与本项目路线相反，主要价值在于提供对比参照。
...
## 4. 潜动作模型（与北极星"看视频掌握玩法"最相关）

- **Genie 潜动作 / LAPO（arXiv 2410.11758）/ VideoWorld / UniVLA / Co-Evolving LAWM**：用 **IDM + FDM + VQ 瓶颈**从**无标注视频**推断离散"潜动作码"，再用少量标注接地到真实控制。
- **对本项目**：本项目已有真实 22 维动作契约 + 逆动力学头，但北极星的"换一套玩法"此前靠**手写**逐 episode 控制重映射（原 `control_remap.py` 已随旧管线退役，待新基座重建）。潜动作文献提示一条升级路线：把它从"手动指定"改为"**每 episode 可推断的潜动作码**"——在 episode 维加一个被 episodic loss 推动的潜变量，给 in-context 适应一个"可学的接口"，对应 [mental_world.md](mental_world.md) 北极星章节点出的架构缺口（inv-dyn 对 context 全盲 / 单 h 太细 / 缺 use-context 梯度压力）。

---

## 5. 推理期速度与 amortization：本项目"退到训练期"是一个合理选择

一个常见问题："既然 DINO-WM / V-JEPA 2-AC / DreamerV3 的核心收益都来自推理期规划，它们怎么做到足够快的反应速度？"调研结论：**全 online MPC 探索本质上慢，真正的速度来自 amortization（把探索蒸馏到反应网络）**。三种情况：

1. **它们大多不需要快**：V-JEPA 2-AC 用 CEM 800 样本、**约 16 秒 / 动作**（比 Cosmos 的 4 分钟 / 动作快约 16×）。机器人抓放可以容忍秒级控制，**不是 20fps 的反应频率**。
2. **DreamerV3 推理期不做探索**：想象 rollout 在**训练期**完成，actor-critic 在想象轨迹上学好；部署时 actor 是**单次 forward** → 实时。**探索被 amortize 进反应策略。** 这是高 Hz 控制的常见做法。
3. **online 也要控制成本**：潜空间（无像素解码）+ 短 receding horizon + **每 K 帧 replan、间隔 open-loop** + GPU 并行候选评估；2026 年出现的 "Amortizing Planning in World Models" 方向把探索蒸馏成快网络。
...
| # | 经验 | 来源 | 对本项目的落点 |
|---|---|---|---|
| 1 | 冻结视觉特征 + ViT 潜预测 + decoder-free 是可行方向 | DINO-WM | 支持核心设计选择；本项目是其超集（多 `c` 闸 + 槽化） |
| 2 | 世界模型可用于推理期潜空间规划，但成本较高 | DINO-WM / V-JEPA 2-AC | 见 §5：本项目"退到训练期"是实时场景下的合理选择，非缺陷 |
