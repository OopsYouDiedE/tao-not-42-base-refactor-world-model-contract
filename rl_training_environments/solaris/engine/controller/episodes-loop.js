...
const {
  SurvivalIronPickEpisode,
} = require("./episode-handlers/survival-ironpick-episode");
const {
  PlaceAndMineEpisode,
...
const episodeClassMap = {
  ...
  survivalIronPick: SurvivalIronPickEpisode,
  // Eval episodes:
...
async function setupBotAndCameraForEpisode(bot, rcon, args) {
  ...
  await sleep(500);
  // no_cheat_items: for from-scratch survival episodes, do NOT hand the bot
  // stone or diamond tools via RCON. Start from an empty inventory so the bot
  // must gather and craft everything itself.
  if (args.no_cheat_items) {
    console.log(
      `[${bot.username}] no_cheat_items=1: skipping stone/diamond-tool grants (from-scratch survival)`,
    );
  } else {
