...
上无限循环用 LoRA 对 **Qwen3-VL 视觉大模型**做行为克隆(SFT)。策略以 Qwen3VL 为视觉
...
环境另需 Java 21。Minecraft 训练需要支持 BF16 的 CUDA GPU（Qwen3-VL-8B 推理约需 17GB
显存）。Qwen3VL 权重公开、无需接受条款；上传 checkpoint 仍需一个具有写权限的 Hugging
...
训练是无限循环，测试与编译校验必须放在训练之前，训练一旦启动其后的命令不会再执行：
    ...
    export HF_REPO_ID=unjustify/minecraft-qwen3vl-vla-10xx
    ...
    python -m train.minecraft.world_model_training \
        ...
        --output "$TAO_STORAGE_ROOT/checkpoints/minecraft-qwen3vl-vla-10xx" \
...
2. 加载 Qwen3-VL-8B（bf16，冻结主干）并注入 LoRA 适配器，只训练适配器参数。
...
Qwen3VL 权重公开、匿名即可下载；只有上传 checkpoint 需要写权限 token，用 `hf auth login`
...
`.cache/huggingface`。Qwen3VL 权重的共享缓存由 `--cache-directory` 指定，上例为
`$HF_HOME/hub`。本地 checkpoint 位于
`$TAO_STORAGE_ROOT/checkpoints/minecraft-qwen3vl-vla-10xx`。`TAO_STORAGE_ROOT` 未设置时
...
Wilson 95% 下界高于 no-op 上界时，`effective_over_noop_95` 才为 `true`：
    ...
    python -m train.minecraft.evaluate_checkpoint \
        --checkpoint "$TAO_STORAGE_ROOT/checkpoints/minecraft-qwen3vl-vla-10xx/last.pt" \
        --dataset-group 10xx \
        --cache-directory "$HF_HOME/hub" \
...
在投入长时间 SFT 前，可先用 `action_token_probe` 探测 Qwen3VL 基座最擅长哪种动作 token
