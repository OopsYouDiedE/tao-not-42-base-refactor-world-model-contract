...
def run_vision_sft(
    ...
    model : str
        完整 Hugging Face 仓库名或本地模型路径。
    dataset_directory : Path
        TAP 预训练数据目录（``build_pretraining_dataset`` 的输出）。
    output_directory : Path
    ...
    streaming : bool
        ...
        False 时读 ``tap_pretraining_dataset`` 的落盘产物。
    ...
    if dataset_path.suffix.lower() in {".h5", ".hdf5"}:
        from dataset.organization.load_hdf5 import load_hdf5_conversations, split_conversations
        ...
        from train.bc.conversation_dataset import load_tap_conversations

        conversations = load_tap_conversations(  # type: ignore[assignment]
            dataset_directory,
