...
if __name__ == '__main__':
    ...
    parser.add_argument("--max-steps", type=int, default=10000, help="Maximum steps to run per env")
    parser.add_argument("--video-path", type=str, default="/home/ame/tao-not-42-base-refactor-world-model-contract/runs/vpt_10k_xvfb.mp4", help="Path to save the video")
    ...
    print(f"创建 {n_envs} 个并行环境（{max_steps} 步/episode，强制 ZEROCOPY 显存直读）...")
    ...
    h, w, _ = obs[0].shape
    video_writer = cv2.VideoWriter(args.video_path, fourcc, 20.0, (w, h))
    video_writer.write(cv2.cvtColor(obs[0], cv2.COLOR_RGB2BGR))

    for step in range(max_steps):
        # 图像 resize 128x128 (使用 numpy array 并一次性丢入 CUDA)
        obs_t = th.from_numpy(np.array(obs)).cuda()  # [B, H, W, C]

        # 视频录制：直接从原生 obs (通常是 640x360) 取一帧并转到 CPU 存入 OpenCV
        if step % 100 == 0:
            print(f"步数 {step}/{max_steps}: 已完成 {len(episodes_rewards_history)} 个 Episodes")
            
        frame_for_video = obs[0]
        # 将 RGB 转换为 BGR 给 OpenCV
        frame_for_video = cv2.cvtColor(frame_for_video, cv2.COLOR_RGB2BGR)
        video_writer.write(frame_for_video)

        # 转换为通道优先 [B, C, H, W] 并缩小尺寸供模型预测
        obs_t = obs_t.permute(0, 3, 1, 2).float()
        obs_t = th.nn.functional.interpolate(
            obs_t, size=(128, 128), mode="bilinear", align_corners=False
        )
        obs_t = obs_t.permute(0, 2, 3, 1)  # [B, H, W, C] 0-255
        ...
        for i in range(n_envs):
            ...
            if i == 0 and "rgb" in obs_dict:
                bgr_img = cv2.cvtColor(obs_dict["rgb"], cv2.COLOR_RGB2BGR)
