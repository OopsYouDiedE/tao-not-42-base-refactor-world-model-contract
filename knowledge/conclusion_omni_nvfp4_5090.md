...
**model card 的 "Minimum GPU (NVFP4): 1× RTX 5090 32GB" 属实。** 但四个坑必须先趟过,
...
> nvcc 只决定能编出什么。坑 3 是驱动侧硬限制(只能换内核),坑 4 是编译器侧的**软性版本断言**(可绕)。
...
- thinking 模式代价 2–11 s,且会跑飞(3000 token 输出空答案)⇒ **不能进控制环**,只适合 episode 级慢系统。

`design_llm_deep_integration.md §1` 给慢系统的预算是 0.5–2 s。**单卡 5090 的 NVFP4 Omni 满足该预算,
且有约 3–10× 余量。**
