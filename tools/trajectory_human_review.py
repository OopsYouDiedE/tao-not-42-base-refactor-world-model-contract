...
TOKEN_EXPLANATIONS = {
    "W": "前进", "S": "后退", "A": "向左移动", "D": "向右移动",
    "space": "跳跃", "shift": "潜行", "ctrl": "疾跑",
    "MouseLeft": "持续挖掘或攻击", "MouseRight": "使用或放置物品",
    "Inventory": "打开或关闭物品栏", "Drop": "丢弃物品",
}
...
def translate_intent(intent: str) -> str:
    replacements = {
        "hold the primary action to mine or attack the visible target": "持续挖掘或攻击画面中的目标",
        "use or place the selected item": "使用或放置当前物品",
        "move forward": "向前移动", "move back": "向后移动",
        "move left": "向左移动", "move right": "向右移动",
        "jump": "跳跃", "sprint": "疾跑", "sneak": "潜行",
...
def format_reference_actions(
    ...
    for index, block in enumerate(blocks, 1):
        ...
        if node_frames is not None and index < len(node_frames):
            ...
            block_label += (
                f" · 图像帧 {block_start} → {block_end}"
                f" · 序列长度 {block_end - block_start} tick"
            )
...
class ReviewStore:
    ...
    def __init__(
        ...
        self.review_path = Path(review_path) if review_path else self.root / "question_reviews.jsonl"
        ...
        self.raw_reader = (
            TrajectoryReader(
                [Path(raw_dataset_directory)], ["action", "image", "meta_info"], 320, 180,
            )
    ...
    def refresh_review_references(self, question: dict[str, Any]) -> None:
        if question["task_type"] not in {"single_frame_intent_to_action", "history_to_future_action"}:
        ...
        array = self.raw_reader.readers["image"].read_frames(
            question["source"]["episode"], frames[1], 1,
        )[0]
        Image.fromarray(np.asarray(array, dtype=np.uint8)).save(
            self.root / end_relative, quality=95,
        )
        question["review_reference_images"] = [
            question["images"][-1], end_relative,
        ]
    ...
    def ensure_review_references(self) -> None:
        ...
        for question in self.questions:
            if (
                question["task_type"] in {"single_frame_intent_to_action", "history_to_future_action"}
                ...
                if current_frames != expected_frames or len(paths) != 2 or any(
                    not (self.root / relative).is_file() for relative in paths
                ):
    ...
    def save_adjustment(
        ...
        actions = self.raw_reader.readers["action"].read_frames(
            episode, action_interval[0], action_interval[1] - action_interval[0],
        )
        ...
        with self._lock:
            for relative, array in zip(question["images"], new_images):
                Image.fromarray(np.asarray(array, dtype=np.uint8)).save(self.root / relative, quality=95)
    ...
    def _rewrite_dataset_files(self) -> None:
        ...
        self._atomic_write(self.root / "ai_review_requests.jsonl", [{
            "id": question["id"],
            "system": AI_REVIEW_PROMPT,
            "question": question,
            "answer": self.answers[question["id"]],
            "output_contract": {"decision": "approve_or_reject", "reason": "one_concise_string"},
        } for question in self.questions])
        self._atomic_write(self.root / "human_review_templates.jsonl", [{
            "id": question["id"],
...
def build_interface(store: ReviewStore) -> gr.Blocks:
    ...
    def render(index: int) -> tuple[Any, ...]:
        ...
        images = [
            ...
            for image_index, (relative, frame) in enumerate(zip(question["images"], frames))
        ]
        details = [f"### {TASK_LABELS[question['task_type']]}", TASK_PROMPTS_ZH[question["task_type"]]]
        if sample_id not in store.reviews and sample_id in store.preannotations:
            ...
            details.extend([
                "### AI 预标注",
                f"建议：**{suggested}**。动作类型：`{preannotation.get('action_class', 'unknown')}`。"
        ...
        if task_type == "image_sequence_to_action":
            ...
            interval_text = (
                f"演示动作区间自动绑定第一张和最后一张图片："
                f"`[{frames[0]}, {frames[-1]}]`。"
            )
        frame_updates = [
            gr.update(value=frames[position] if position < len(frames) else 0, visible=position < len(frames), interactive=can_adjust)
        ...
        future_gallery = (
            ...
            if len(reference_frames) == 2 and len(reference_images) == 2 else []
        )
        ...
        return (
            ...
            display_sequence, *frame_updates,
            gr.update(
                value=interval[1],
            ...
            interval_text,
            "✅ 可从 7xx 原始数据重新取帧。" if can_adjust else "ℹ️ 缺少对应原始 LMDB，当前题目只读。",
            "\n\n".join(details),
            ...
            review.get("reason", ""), progress_markdown(store),
        )
    ...
    def save_review(index: int, intent: str, decision: str, reason: str) -> tuple[str, str]:
        ...
        store.save({
            "id": question["id"], "decision": decision, "reason": concise_reason,
            "reasons": [concise_reason], "review_kind": "human_question_review",
    ...
    def adjust_frames(index: int, *values: Any) -> tuple[Any, ...]:
        ...
        try:
            store.save_adjustment(question["id"], frames, int(target_end) if target_end is not None else None)
    ...
    with gr.Blocks(title="MineStudio 轨迹题目审核") as interface:
        ...
        with gr.Row():
            ...
            sample_selector = gr.Dropdown(
                ...
                value=store.questions[0]["id"], label="跳转到题目", filterable=True, scale=4,
            )
        title = gr.Markdown()
        gallery = gr.Gallery(
        ...
        render_outputs = [
            index_state, sample_selector, title, gallery, *frame_inputs, target_end,
            interval_description, adjustment_help, prompt, reference_actions, intent_input,
            decision, reason, progress,
        ]
        ...
        save_next_button.click(save_review, save_inputs, [status, progress]).then(
            lambda index: navigate(index, 1), index_state, render_outputs,
        )
        apply_adjustment.click(
            adjust_frames, [index_state, *frame_inputs, target_end], [*render_outputs, adjustment_status],
        )
    return interface
