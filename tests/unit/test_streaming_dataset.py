...
def test_worker_count_falls_back_to_cores_when_memory_unknown(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """内存读不到时只按核心数决定，不猜测内存大小。"""
    from machine_environment import hardware_report

    monkeypatch.setattr(
        hardware_report, "collect_memory_report", lambda: hardware_report.MemoryReport(),
    )
