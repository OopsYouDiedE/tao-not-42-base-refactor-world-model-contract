...
* **观察**：tick 开头的 `Observe` 表示在该 tick 输入生效前立即截图，并以该观察异步启动下一轮推理，且不占用 tick。`Observe` 所在 tick 及其后的旧序列动作是生成当前序列的模型预先提供的延迟填充，不是基于新观察生成的动作。环境在等待新结果时继续执行填充；新结果到达后，从当前 tick 覆盖尚未执行的旧填充，已经执行的填充不回滚。`Observe` 与 `xN` 同用时只截图一次，例如 `Observe W x8` 表示触发异步重规划，并预先提供最多 8 tick 的 `W` 作为延迟填充。
...
| `Observe` 不在 tick 开头或没有后续动作 | 发出 `RuntimeWarning`，该段按一个 `NoOp` tick 处理 |
