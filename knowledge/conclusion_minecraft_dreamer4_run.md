...
1. **重建税**：生成路径过 tokenizer 有损瓶颈,动力学再完美天花板也是 psnr_recon;
   ...
   动力学误差从这点预算里扣。
2. **静止像素灌水**：30Hz 下 8 帧≈0.27s,天空/HUD/远景几乎不动,persistence 在
...
EV=-0.323(基线 +0.087)、开环 -4.10dB(基线 -1.15)。3500 步即杀,省 ~1.4h GPU。

**解读**:按自身流误差加权 = 把权重压向高运动/难转移,早期模型在难样本上梯度
方差本就大,放大 16 倍(0.25→4)直接破坏了流匹配的收敛;开环崩得最狠(-4.1dB)
