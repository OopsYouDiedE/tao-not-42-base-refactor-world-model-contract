...
try:
    import lmdb
except ModuleNotFoundError:  # 元数据算法可在不安装 LMDB 的环境中独立使用。
...
class LMDBModalityReader:
    ...
    def _register_part(self, directory: Path) -> None:
        ...
        if lmdb is None:
            raise ModuleNotFoundError("读取 MineStudio 分片需要安装项目依赖 lmdb")
    ...
    def read_frames(self, episode: str, start: int, length: int) -> Any:
        ...
        missing = [frame for frame, chunk in zip(wanted, decoded, strict=True) if chunk is None]
        if missing:
            environment = self._environments[info.part_directory]
            with environment.begin(write=False) as transaction:
                for position, (chunk_frame, chunk) in enumerate(zip(wanted, decoded, strict=True)):
...
class TrajectoryReader:
    ...
    dataset_directories : list of Path
        数据集根目录列表，例如 ``[Path("runs/datasets/minestudio-data-10xx-v110")]``。
        ...
        for modality in modalities:
            ...
            if not parts:
                raise FileNotFoundError(
                    ...
                    "dataset.extraction.minestudio.download 下载",
                )
    ...
    def episode_length(self, episode: str) -> int:
        ...
        return min(reader.episode_info(episode).num_frames for reader in self.readers.values())
