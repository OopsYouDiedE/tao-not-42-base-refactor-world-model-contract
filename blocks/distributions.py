"""L1 primitive 积木库 - 概率分布参数化与符号对数变换 (blocks/distributions.py)。

对外接口:
    symlog / symexp                — 符号对数及其逆(压缩重尾标量目标)。
    SampleDist                     — 用蒙特卡洛样本近似 mean/mode/entropy 的分布包装。
    OneHotDist                     — 带 unimix 混合 + 直通梯度的 one-hot 类别分布。
    DiscDist                       — two-hot symexp 离散分布(reward/value 头用)。
    MSEDist / SymlogDist           — 以 (sym)MSE 为 log_prob 的确定性分布(图像/向量重建)。
    ContDist / Bernoulli           — 连续/伯努利分布的 mode/sample/log_prob 包装。
    UnnormalizedHuber              — Huber 似然的 Normal 子类。
