...
本子包是按节点截单帧的 mineflayer 采集。solaris 那条"连续录制 mp4 + 逐帧 22 维动作契约 +
prismarine-viewer 无头渲染"的路是独立环境,已迁到 `rl_training_environments/solaris/`
（含 vendored engine、负 y 地形补丁、动作起止截图验收）。两者都是 mineflayer 行为数据源,
但 solaris 不属于本子包。
