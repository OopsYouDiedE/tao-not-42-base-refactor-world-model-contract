...
from online_interactive_environments.godot.godot_process import (
    launch_godot,
    terminate_godot_process,
)
from online_interactive_environments.godot.ppo_model_factory import build_ppo_model
from online_interactive_environments.godot.vectorized_environment import (
...
def _parse_args():
    ...
    parser.add_argument(
        "--godot-exe", default=None, help="Godot 4 .NET 可执行文件；默认读取 GODOT_EXE"
    )
...
def main():
    ...
    try:
        if not arguments.no_launch:
            launch_arguments = (
                {} if arguments.godot_exe is None else {"godot_exe": arguments.godot_exe}
            )
