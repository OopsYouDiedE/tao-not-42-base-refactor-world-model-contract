...
def test_default_settings_produce_minimum_rolling_plan() -> None:
    """默认布局产出滚动执行要求的 8 tick、400ms 计划。"""
    settings = StreamingSettings()
    assert settings.window_frames == 8
    assert settings.stride_frames == 8
...
def test_sample_positions_do_not_run_past_episode_end() -> None:
    ...
    assert positions == [("ep", 0)]  # start=8 时 8+8>10，丢弃


def test_sample_positions_reserve_history_span() -> None:
    """有历史帧时起始帧后移，保证回溯不越界。"""
    settings = StreamingSettings(history_windows=2)
    positions = _sample_positions({"ep": 30}, ["ep"], settings)
    assert positions[0] == ("ep", 16)  # 2 个窗口 × 8 帧
    assert all(start >= 16 for _, start in positions)
...
def test_worker_count_leaves_a_core_for_main_process() -> None:
    ...
    assert (
        resolve_worker_count(
            logical_cores=8,
            available_memory_bytes=64 * _GIBIBYTE,
        )
        == 7
    )
...
def test_worker_count_is_capped_by_memory() -> None:
    ...
    assert (
        resolve_worker_count(
            logical_cores=32,
            available_memory_bytes=4 * _GIBIBYTE,
        )
        == 4
    )
...
def test_worker_count_respects_hard_ceiling() -> None:
    ...
    assert (
        resolve_worker_count(
            logical_cores=256,
            available_memory_bytes=1024 * _GIBIBYTE,
        )
        == 16
    )
