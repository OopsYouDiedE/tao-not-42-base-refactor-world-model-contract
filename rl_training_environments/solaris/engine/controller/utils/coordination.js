...
/**
 * Single-bot loopback coordinator.
 *
 * The real BotCoordinator is a TCP peer-to-peer channel between two bots; every
 * phase is a ping-pong where a bot registers a listener then sends the event to
 * its peer, and the listener only fires when the peer echoes back. With a single
 * bot there is no peer, so setupConnections() and every sendToOtherBot() would
 * deadlock. This subclass makes the lone bot play both sides: connections resolve
 * immediately, and every "send to peer" is re-emitted locally (deferred via
 * setImmediate to mimic async socket delivery), so the bot answers its own
 * handshakes and the episode lifecycle proceeds exactly as in the two-bot case.
 */
class LoopbackCoordinator extends BotCoordinator {
  async setupConnections() {
    console.log(`[${this.botName}] LoopbackCoordinator: no peer, self-answering`);
    return { serverReady: true, clientReady: true };
  }

  sendToOtherBot(eventName, eventParams, episodeNum, location) {
    const fullEventName = getEventName(eventName, episodeNum);
    console.log(
      `[loopback] ${location}: self-emitting ${fullEventName}`,
    );
    // Defer so the current handler finishes before its paired listener fires,
    // matching the real coordinator's asynchronous socket delivery.
    setImmediate(() => this.emit(fullEventName, eventParams));
  }
}
