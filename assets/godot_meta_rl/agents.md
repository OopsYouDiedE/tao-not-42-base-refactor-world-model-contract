...
### `model_base.gd`
* **作用**：所有元学习强化学习采集环境的【基础框架类】。封装了公共的动作空间（10连续+30离散）、相机云台控制、运动阻尼与夹紧逻辑、时间计算及统一的 Python-C# 契约交互接口。
* **方法标注与调用关系**：
...
| `get_reward_done() -> Vector2` | C# 专属高效合一接口：以 Vector2 一次性返回缓存的 reward 和 done，免除两次跨语言调用。 | <li>`Main.cs` -> `PublishObservation()`</li> |
...
| `_compute_reward() -> float` *(虚函数)* | 子类计算当前渲染帧稠密/稀疏奖励的重写入口。 | <li>`model_base.gd` -> `step_render()`</li><li>`model_base.gd` -> `_physics_process()`</li> |
...
* **作用**：具体任务场景代码——【聚光灯瞄准·离散控制版】。继承自 `ModelBase`。实现了随机生成目标物品、随机时刻触发聚光灯照亮特定物体、计算相机与物体的夹角误差、执行离散键位运动控制以及两阶段差分加稠密惩罚奖励机制。
* **方法标注与调用关系**：
...
| `_build_room() -> void` | 拼装包含天花板、地板及红绿蓝黄不同颜色墙壁的六面房间盒子，给墙壁赋予棋盘格纹理以提供明确的旋转视觉定位线索。 | <li>`env_spotlight_discrete.gd` -> `_build_world()`</li> |
| `_make_checker_texture() -> ImageTexture` | 算法生成一张 32x32 灰白棋盘格纹理图。 | <li>`env_spotlight_discrete.gd` -> `_build_room()`</li> |
| `_dir_from(yaw: float, pitch: float) -> Vector3` | 基础几何函数，将球面角坐标转为三维直角方向向量。 | <li>`env_spotlight_discrete.gd` -> `_reposition_targets()`</li> |
| `_reposition_targets() -> void` | 重随机各候选障碍物在安全仰角下的三维空间位置。 | <li>`env_spotlight_discrete.gd` -> `_reset_task()`</li> |
| `_reset_task() -> void` | 充当任务重置，将聚光灯置为不可见，随机生成照亮发生的延迟时刻 `_trigger_time`，并重排场景内所有候选物体的位置。 | <li>`model_base.gd` -> `reset()`</li> |
| `_light_target() -> void` | 核心触发函数。随机抽选某一目标物体，将其表面用 SpotLight3D 强光打亮，开始记录其初始角误差和重设差分跟踪。 | <li>`env_spotlight_discrete.gd` -> `_task_physics()`</li> |
