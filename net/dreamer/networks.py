"""DreamerV3 的网络部件:RSSM 世界模型核心 + 多模态编/解码器 + MLP 头。

对外接口:RSSM、MultiEncoder、MultiDecoder、ConvEncoder、ConvDecoder、MLP。

来源:原样照抄 NM512/dreamerv3-torch 的 `networks.py`(MIT,见 net/dreamer/NOTICE)。
按本仓 blocks/net 分层约定,原文件中的底层算子已物理拆出:GRUCell→blocks.dynamics、
Conv2dSamePad/ImgChLayerNorm→blocks.conv、tools.* 经 net.dreamer._compat 重聚合;
本文件仅改了顶部 import,以下网络类体保持 1:1 原样。
"""
import math
import numpy as np
import re
...
from blocks.dynamics import GRUCell
