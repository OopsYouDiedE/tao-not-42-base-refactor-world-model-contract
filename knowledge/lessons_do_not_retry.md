...
- 别再用单帧全局相位相关在人类录像上做离线相机标定 — 静态覆盖层(HUD/F3 调试屏/手持地图)
  把全局峰锁死在零位移,快转头帧运动模糊使世界纹理弱于覆盖层,实测增益衰减 ~7×
  (0.021 vs 期望 0.15 deg/px)、相机主导帧比值中位数≈0 — 证据:tests/probe_vpt_calib.py 顶部实测记录。
- 别再读屏取特权调试信息当标定/监督源(如 OCR F3 调试屏的 yaw 真值)— 用户裁决禁令
  (2026-07-10):给特定游戏的调试布局写解析管线 = 领域先验补丁,与退役的凸包 GT/手标分割头
  同类;物理参数测不出就置 None(net/calibration.py 立场),数据集格式常量可当 schema 知识
  引用并注明上游出处 — 证据:docs/activity_log.md(2026-07-10 BC 暖启动批)。
