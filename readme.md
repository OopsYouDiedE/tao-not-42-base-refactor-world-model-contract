...
solaris / prismarine-viewer（每个动作起止时截图 + 全程录像）：先用 solaris 引擎跑出
逐帧动作 JSON + 录像 mp4（引擎在独立仓库，流程见 `SETUP_WSL_SMOKE.md`），再后处理抽帧：
...
## 目录
    ...
    data_pipelines/mineflayer_actions/action_boundary_shots.py  逐帧动作 json + mp4 → 动作起止截图
