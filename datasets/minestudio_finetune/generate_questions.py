...
def infer_action_intent(actions: dict[str, np.ndarray]) -> tuple[str, str] | None:
    ...
    active = {
        field for field, values in actions.items()
...
def build_questions(
        ...
        for task_type in TASK_TYPES:
            ...
            while generated < samples_per_type and attempts < samples_per_type * 500:
                ...
                questions.append(build_question_record(
                    sample_id, task_type, episode, start, images, raw_actions, intent,
                ))
                answers.append({
                    "id": sample_id,
                    "task_type": task_type,
    ...
    manifest = {
        ...
        "shortfall_per_type": {
            ...
            for task_type, count in generated_per_type.items() if count < samples_per_type
        },
        "seed": seed,
        ...
        "automatic_filters": [
            ...
            "insufficient_visual_change", "gui_change_without_click", "static_or_weak_action",
        ],
