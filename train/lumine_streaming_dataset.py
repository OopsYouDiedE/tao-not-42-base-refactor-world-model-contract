...
对外接口：
    ...
    LumineStreamingDataset — PyTorch ``Dataset``，逐样本从 LMDB 转出对话。
...
class LumineStreamingDataset:
    """从 LMDB 流式产出 Lumine 对话样本的 PyTorch ``Dataset``。
...
) -> tuple[LumineStreamingDataset, LumineStreamingDataset, dict[str, Any]]:
