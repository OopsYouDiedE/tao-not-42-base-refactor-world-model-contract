...
> **文档定位（SSOT）**：本文只讲**宏观架构、算法意图、训练目标的设计动机**。
> 易变的实现细节（精确 Shape/Dtype、层数、超参）归代码本身：
> 冻结骨干见 [net/backbone.py](../net/backbone.py)，积木见 [blocks/](../blocks/)，
> 数据契约归 [train/minecraft/](../train/minecraft/)（`vpt_action`/`vpt_dataset`/`task_text`）。
> （旧活模型 `net/world_model.py` 及 `slots`/`heads` 已随退役管线删除，新基座 `net/` 与训练循环实现待补——见下方退役公告。）
> 本文是设计锚点，记录"为什么这么搭"，不记录历史活动（历史活动归 git log 与项目记忆）。

> **退役公告 + 读法（2026-06，统一世界基座重设计）**：本文 §3–§5、§8 描述的
...
> 二者被重新设计的统一跨域世界基座取代；新基座设计文 `knowledge/world_foundation.md` 待构建期补入。
> 在新设计落地前，§3–§5、§8 仅作**已退役方案的历史记录**阅读，不代表当前代码。
> - **仍有效的设计锚点**：§0–§2（脑内世界立场、有限持久潜向量、与 LeWM 的区别）、§6（北极星：看视频掌握玩法）、
>   §7（选择性读取愿景，无代码）——这些是"为什么这么搭、往哪走"的设计意图，新基座继承。
> - 读"当前能诚实主张什么、离目标多远、值不值得继续"以 [claims_and_scope.md](claims_and_scope.md) 为准（期望-现实校准）。
...
- 残余的"多个 slot 冗余绑同一区域"由**结构 + 损失**两层处理：结构上 `SlotBinder(compete=True)` 用 `SlotCompetitiveAttn` 沿 **slot 维**做零和竞争（一个 patch 的注意力质量被各槽分摊）；损失上 `slot_diversity_loss`（`--beta_div`）软惩罚竞争注意力图的成对重叠。约束加在**注意力空间（谁看哪里）**，前向注意力仍是合法分布；**不**在前向里硬正交化（施密特那样硬改聚合权重会破坏 `out=w·v` 的加权平均语义，并按 slot 序让靠后的槽缺少输入）。
...
**六条真区别（本设计的选择）**，并标注当前落地状态（已落地 = 活模型已实现；部分 = 部分维度落地；愿景 = 无对应代码）：
...
本节描述原 `net/world_model.py`（已退役删除）的 `MinecraftWorldModel` 结构，是 §0–§2
...
4. **序列↔序列对齐**：`L_align = Σ_k (1/M)Σ_p w·‖ẑ^{(k)}−z̄‖² + λ_agree·Σ_{k<k'}‖ẑ^{(k)}−sg ẑ^{(k')}‖²`
   ——多上下文预测同一 t* 且互相一致。
5. **后果权重**：`w ∝ norm(‖ẑ_inv(do a)−ẑ_inv(do null)‖ + α·Φ_future)`，随未来效应而非像素变化 ⇒ 小像素高后果不被淹没。
...
- **因子化潜 z=(z_rev,z_inv)**：`z_rev` 经 `BoundedActivation('flow')` 有界（I3）；`z_inv` 由 `StochLatent` 给随机潜 + `β_kl·KL` 信息瓶颈（逼其只留会改变未来的部分；β_kl 不能过大以免丢掉关键信息，由后果监督托底）。
- **EMA 教师 + stop-grad（I8）**：目标走 `adapter` 的 EMA 副本 `target_adapter`，慢速跟踪；降低预测误差不能靠移动目标实现。
...
**训练采样**：每个 clip 取锚点 0、目标 t*=末帧、若干上下文截止 k；构造 token 集合一次前向算对齐 + 一致损失，EMA 每优化步更新。（旧实现 `run_sequence` 已随退役管线删除，新基座待重建。）

---

## 4. 可控性与不确定性（后果权重与因子化潜）

- **后果权重 w（已落地）**：利用反事实效应 $\|e\| = \|\hat{z}_{inv}(do\ a) - \hat{z}_{inv}(do\ null)\|$ 衡量一个动作对世界的影响程度。空中跳跃或无效按键会产生 $\|e\| \approx 0$，而合成、破坏方块等会产生较大的 $\|e\|$。以此计算后果权重 $w$ 进行重加权，只参与损失加权不反传梯度（detach），使模型关注低频但高影响（低像素能量、高后果）的模式，缓解小像素高后果被像素 MSE 淹没的问题。
- **因子化潜 z=(z_rev, z_inv) 与 KL 信息瓶颈（已落地）**：潜空间被因子化为可逆通道 $z_{rev}$ 与不可逆通道 $z_{inv}$。
  - $z_{rev}$ 通过 BoundedActivation 和可逆生成元算子组限制发散；
  - $z_{inv}$ 通过 `StochLatent` 模块注入 $\beta_{kl} \cdot KL$ 散度信息瓶颈，使模型丢弃不改变未来的冗余信息，只保留具有不可逆后果的部分。
...
**全程自监督**（无人工标注）：预测 target 来自慢速跟随的 EMA 教师目标编码器 `target_adapter` 的输出；通过可变跨度 $\Delta t$ 的 jumpy 采样以及多上下文截止集合，在序列空间中一次前向构建对齐和一致性约束。
...
3. **EMA 目标慢速跟随**：教师权重以大动量（如 0.996）滑动更新，保证目标稳定。

**两条跨任务设计约束**：
- **可变时间步跳跃采样**：转移跨度 $\Delta t \sim U\{1..max\_skip\}$ 帧，不预测中间冗余状态，只由累积动作与跨度时间编码直接对齐到未来。
...
- **脚手架**：控制重映射（逐 episode，零新数据即制造"同一画面、不同操作语义"的 in-context 信号；原 `control_remap.py` 已随旧管线退役，待新基座重建）；[task_text.py](../train/minecraft/task_text.py)（任务条件，已就位）。
- **证据档**：① 逐 episode 操作重映射（看一段被重映射的视频 → 推出本 episode 的操作语义）；② episodic loss-on-query（在 query 段上算适应增益）。
- **评估协议**：baselines（无记忆 / 随机 h）对照 + dose-response（适应增益随 context 视频长度单调）。真 Minecraft 活环境（craftground，headless 软渲染）解锁主动交互轴 + rollout 通关率，离线数据测不了这些。
...
本节描述**尚未落地**的设计愿景，**当前仓库无对应代码**（原 `tao_not_42.py` 的 fovea/gaze 实现已删）。保留它是因为：当"读取成本"成为瓶颈时，这是既定演进方向。
...
- **训练靠两阶段自监督蒸馏（绕开 RL）**：阶段一全观测训出动力学与表征；阶段二冻成老师，gaze/wake 用**从自身预测误差算出的 oracle 当自监督标签**（误差热图监督看哪里、想象 rollout 首次发散时刻监督何时看）。**观测必须有代价** `cost(obs)`——否则"最大降低预测误差"的最优解是全看，退回稠密感知；加入代价才能逼出稀疏读取。
...
- **薄探针原则**：检验"Z 是否真存了世界"的探针须**刻意保持简单**（单隐层），且从**聚合后的全体残差潜向量**读出、非某个固定 slot——读得准只能归功于 Z 本身确实编码了世界信息，而非探针自行拟合。（原 `net/world_probe.py` 的 `WorldProbeDecoder` 是这类探针的实现，已随旧管线退役，待新基座重建。）
- **诊断手段（旧实现已随退役管线删除，待新基座重建）**：
  - 逆动力学上界——冻结特征里到底能读出多少动作，定位瓶颈在编码还是读出。
  - 开环 rollout 漂移：累积 `ẑ+μ` 与真相的发散曲线，量化前向模拟能撑多久。
- **指标约束**：① 低 loss 不等于世界模型有效，须用抗作弊指标（开环对照，骗不了位置先验）；② 多实体读出结构化 query、不池化（§5）；③ 运动样本要单独看（全步 RMS 会被静止帧稀释）。
...
6. **VPT teacher 蒸馏**（teacher 适配器 `vpt_teacher.py` 与蒸馏训练循环 `distill_vpt.py` 均已随退役管线删除，待重建）：软 KL 把 OpenAI VPT 策略边缘化到 22 维契约，给动作头一个强先验。
