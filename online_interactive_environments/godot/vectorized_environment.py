...
from online_interactive_environments.godot import (
    shared_memory_environment as environment_contract,
)
...
class RolloutProgress(BaseCallback):
    ...
    def _on_rollout_end(self):
        ...
        print(
            f"[progress] steps={self.num_timesteps:>7d}  ep_rew_mean={rew_mean:+.3f}  "
...
class GodotVectorizedEnvironment(VecEnv):
    ...
    def __init__(self, connect_timeout_s=60.0):
        self.env = environment_contract.GodotTrainingEnvironment(
    ...
    def step_async(self, actions):
        self._actions = np.asarray(actions, dtype=np.int32).reshape(
            environment_contract.NUM_ENVS, N_BUTTONS
        )
    ...
    def _read_obs(self):
        ...
        sim_dt = (
            self._meta[:, environment_contract.M_SIM_DT : environment_contract.M_SIM_DT + 1]
            .astype(np.float32)
            .copy()
        )
