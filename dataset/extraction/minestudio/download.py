...
def download_datasets(
    ...
    for dataset in datasets:
        ...
        if clean and target.exists():
            ...
            if resolved.parent != output_directory.resolve() or not resolved.name.startswith(
                "minestudio-data-"
            ):
        ...
        if "image" in modalities and maximum_image_parts is not None:
            ...
            image_parts = sorted(
                {"/".join(path.split("/")[:2]) for path in files if path.startswith("image/part-")},
                key=lambda part: int(part.rsplit("-", 1)[-1]),
            )[:maximum_image_parts]
            if not image_parts:
