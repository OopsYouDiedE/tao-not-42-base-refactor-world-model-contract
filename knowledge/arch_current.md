...
> ⚠️ **2026-07-10 用户拍板:视觉前端定 DINO patch 网格,YOLOE 整线废弃并已删码**
> (objectness 头本身是人工监督先验,地形/洞/坡在提案表示里结构性缺席)。
> 本节的 §2.2 词表塌缩数学与实测**仍是有效证据**(被登记表引用),保留作档案;
> §2.1/§2.3 的 YOLOE 打分式与路线 2 token 契约**作废**,现行前端契约见
> `design_bitter_lesson_map_integration.md §7`(DINO patch + fovea 双尺度)。
> §1.2 的 goal 换基契约同理作废(goal-as-query 思想由 token_tower 继承,基不再是 e_j 空间)。
> **行号悬空提示(2026-07-10)**:本节与 §1.2/§6.6 引用的 `net/fovea_twotower/yolo_unified.py` 等文件已随删码消失,
> 这些 `文件:行号` 对应删码前的 git 历史(commit `f6e997b` 及之前),现行仓库无法核对;
> 原文查 git 历史（commit `f6e997b` 及之前）。

本节数学从已删除的 YOLOE 实现与设计档案抄准；原文见 git 历史（commit `f6e997b` 及之前）。

### 2.1 native 打分式

YOLOE 的 promptable 对比头（BNContrastiveHead）逐锚打分如下；对应退役实现见 git 历史：
