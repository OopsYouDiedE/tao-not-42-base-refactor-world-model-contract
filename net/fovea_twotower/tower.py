...
class ContextTower(nn.Module):
    ...
    def __init__(self, d=384, layers=12, heads=6):
        super().__init__()
        self.d = d
    ...
    def interleave(self, lat, act):
        """lat [B,L,81,384], act [B,L-1,24] → [B,L*82-1,d](末帧无动作)。"""
        B, L = lat.shape[:2]
        v = self.vis_in(lat) + self.type_emb.weight[0]           # [B,L,81,d]
        if L == 1:
            return v[:, 0]
        a = (self.act_in(act) + self.type_emb.weight[1])[:, :, None]  # [B,L-1,1,d]
        body = torch.cat([v[:, :-1], a], 2).flatten(1, 2)        # [B,(L-1)*82,d]
    ...
    def forward(self, lat, act):
        """训练:下 token 潜变量 MSE。"""
        B, L = lat.shape[:2]
        x, _ = self.backbone(self.interleave(lat, act))
        pred = self.head(self.norm(x))                 # [B,T,384]
        T = x.shape[1]
        is_vis = torch.ones(T, dtype=torch.bool, device=x.device)
        is_vis[N_PATCH::N_PATCH + 1] = False           # 动作位
        ...
        return F.mse_loss(pred[:, :-1][:, m], tgt_flat[:, 1:][:, m])

    @torch.no_grad()
    def encode(self, lat, act, want_states=False):
        """返回 (hidden [B,T,d], states|None);states=[{recurrent_state,conv_state}]×9。"""
        cache = Cache() if want_states else None
        x, cache = self.backbone(self.interleave(lat, act), cache)
        states = None
        if want_states:
...
class ActionTower(nn.Module):
    ...
    def __init__(self, d=384, layers=12, heads=6, horizon=8):
        super().__init__()
    ...
    def forward(self, lat_now, x_tau, tau, seed=None):
    ...
    @torch.no_grad()
    def sample(self, lat_now, seed=None, steps=4, generator=None):
        ...
        for i in range(steps):
            tau = torch.full((B,), i / steps, device=lat_now.device,
                             dtype=lat_now.dtype)
            x = x + self.forward(lat_now, x, tau, seed) / steps
        return x
