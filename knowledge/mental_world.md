...
- **薄探针原理**：检验"Z 是否真存了世界"的探针须**刻意保持极薄**（单隐层），且从**聚合后的全体残差潜向量**读出、非某个固定 slot——读得准只能归功于 Z 本身确实存了世界，而非探针脑补。（原 `net/world_probe.py` 的 `WorldProbeDecoder` 是这类探针的实现，已随旧管线退役，待新基座重建。）
...
6. **VPT teacher 蒸馏**（teacher 见 [train/minecraft/vpt_teacher.py](../train/minecraft/vpt_teacher.py)；蒸馏训练循环 `distill_vpt.py` 已随退役管线删除，待重建）：软 KL 把 OpenAI VPT 策略边缘化到 22 维契约，给动作头一个强先验。
