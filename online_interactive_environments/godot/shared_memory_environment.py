...
# 仓库根：online_interactive_environments/godot/ 向上两级。
...
class GodotTrainingEnvironment:
    ...
    def warmup(self, timeout_ms=120000, frames=2):
        ...
        for _ in range(frames):
            if not self.wait_for_observation(timeout_ms):
                return False
            self.send_actions(
                np.zeros((NUM_ENVS, CONT_DIM), np.float32), np.zeros((NUM_ENVS, DISC_DIM), np.int32)
            )
        return True
    ...
    def read_image_observations(self):
        ...
        return np.frombuffer(raw, dtype=np.uint8).reshape(
            NUM_ENVS, IMAGE_HEIGHT, IMAGE_WIDTH, CHANNELS
        )
