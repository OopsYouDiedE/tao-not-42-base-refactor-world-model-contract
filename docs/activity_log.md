...
- **四组齐，判定与选型已合并到 knowledge/README.md**：
...
- 设计结论写入 `knowledge/README.md`：分层吃法（tokenizer 吃原生
...
- 用户拍板 W/C/A 三模型架构,设计入库 knowledge/README.md
...
- gaming500_mc 动作数据验收通过(此前"按键全空"为探针字段用错的误报,实际 50.2%
...
- **g500 训练两次 OOM 根因修复**:gaming500 单段 30 分钟,128² uint8 整段缓存
  2.6GB/段,旧参数 (9w×6c) 需 ~140GB;第二次 OOM 发生在 ckpt 保存的 ~8GB CPU 拷贝峰值。
...
- 16:21-16:30 **封片策略缺陷确认与修复**:shard_0000 膨胀至 8.9GB 仍未封——旧策略等
  ...
  插曲:pkill+relaunch 同壳执行时 pkill -f 命中含相同文件名的包装 shell 误杀自身,拆两步解决
...
### 故障记录（四个，全部与 sm_120 + 570 驱动强相关）

- **问题 0：PyPI 默认轮子是 CUDA 13 构建。** `vllm==0.20.0` 依赖 `torch==2.11.0`，而 PyPI 默认
...
- **问题 1：model card 的 `--moe-backend triton` 对 NVFP4 权重是无效建议。**
...
- **问题 2：视觉编码器的 FlashAttention-2 撞 PTX 工具链版本墙。**
  加载期 `profile_run` 抛
...
- **问题 3：FlashInfer 的 sm_120 JIT 有个过严的版本守卫。**
  ...
  > 教训（"驱动版本"≠"nvcc 版本"：问题 2 是驱动侧硬限制只能换内核，问题 3 是编译器侧软性版本断言可绕）
  > 已沉到 `knowledge/README.md §1`。

### 落地产物
- `tests/serve_omni_nvfp4.sh` —— 可复现的启动脚本，四个问题全部内联注释。
...
结论沉到 `knowledge/README.md`，此处只记过程与碰壁。
...
- 无 java / 无 Xvfb ⇒ `apt-get install openjdk-21-jdk xvfb`。GL 走 Xvfb + llvmpipe（OpenGL 4.5 core，
  ...
  避免它的 numpy/protobuf 破坏 vllm 的 venv。
- **故障 1**：CraftGround 运行时用 cmake 编原生扩展，缺 GL 开发头文件 ⇒ `Could NOT find OpenGL`。
...
- **故障 2**：`reset()` 返回的首帧是 **"Loading terrain..."** 加载画面。喂给 VLM 是纯噪声。
  必须空跑 ~60 tick 等地形。
- **故障 3**：`InitialEnvironmentConfig` 的方法签名与直觉不同——
...
- **故障 4（助手自身的 bug）**：初版 oracle 用 `ORACLE[d % 5]`，每 5 个决策重发一次 `pitch +60`，
...
`knowledge/README.md §5`；原始数据 `docs/results/omni_history_latency.json`。
...
llvmpipe 分不到 CPU——软件渲染吞吐强依赖空闲 CPU。）
...
## 2026-07-09（Vast 5090，会话 1 续 2：慢塔接线 + 三次评价体系失误）
...
2. **慢塔指点体检(v1)**:我在 system prompt 里给了**带具体坐标的示例**
   ...
   → **对照臂证伪**:恒定复读 `[430,560]` 也是 0.75;随机瞄准在该失真指标上甚至有 0.438,
...
## 2026-07-09（会话 1 续 3：n_cls 的代价被量化——词表使像素覆盖 90.8% 塌到 1.3%）
...
### 新问题：torchvision 在 sm_120 上没有 CUDA 内核
...
```
⇒ **YOLOE 在 RTX 5090 上默认安装即失败。** 临时绕过：把 NMS 搬到 CPU（框数仅数百，代价可忽略，
见 `tests/probe_yoloe_coverage.py::patch_nms_to_cpu`）。用户计划换 cu130 机器，届时应自愈。
（与今早 vLLM 的三个 sm_120 问题同类：生态尚未跟上 Blackwell 消费卡。）
...
**重设计**:与用户逐轮敲定,定稿在 `knowledge/README.md`
...
- **删除 5**(零入引/纯过时计划/错误现状快照,git 历史可查):`knowledge/README.md`
  (把退役 dreamerv3 标"当前最活跃",误导最重)、`design_learned_attention.md`、
  `design_gaming500_hd_pretrain.md`、`design_gaming500_consume.md`、`install_refactor.md`。
- **打废弃头 9**(有误导性但被现行文件/登记表引用或混有有效证据,封存不删):
...
- 已知遗留:`knowledge/README.md:605` 引用不存在的 `knowledge/README.md`(失效链接,未修)。
...
- **knowledge/README.md 重写**(611 行旧全模块指南→当前主线三依赖 + 冒烟自检;
  旧版 git 历史),原 ppo_ad.md 失效链接随重写消失。
- `knowledge/README.md` 随代码同批删除;`knowledge/README.md` 对应行改标"已删除"。
...
- 文档裁决同步:papers_in_use(YOLOE→退役清单,新增 DINOv3 方向行)、
  ...
  status_built_not_wired、next_session §7、设计文档 §7/§8/§12、knowledge/README.md。
...
- 结果(单环境 640×360×500 step,同一动作序列):Xvfb-RAW 13.1 sps / Xorg-RAW 37.3 sps /
  **Xorg-ZEROCOPY 25.3 sps(obs 直落 cuda:0,跑通)** / EGL 无 X:驱动层可用、CraftGround 栈不支持。
  数据与判读回填 `knowledge/README.md §3`(旧"ZEROCOPY 未启用"表述就地更新)。
...
- **训练**(L4,batch 96=1536 tick/step,bf16):发现帧堆叠 CPU 展开使 GPU 闲置
  ...
  判读与逐键表入 knowledge/README.md 末节(覆盖写,未新建文件)。
- **渲染基准**(并行 SubAgent,fdbb43e):四臂结果回填 knowledge/README.md §3
...
- **真跑 4×4×400(BC 暖启动 + 真慢塔 + 判官首次完整闭环)**:判官 4/4 真排序
  ...
  knowledge/README.md（现压缩于 §3、§6.2）。
...
- 结论覆盖写入 knowledge/README.md(五 run 动力学、run5 指标列、200:1 规律)。
...
- **A/B 定案(用户裁决),主 run 判官切 Sonnet-low**:退化臂两模型都 6/6 并列(Sonnet
  补测同 PASS);真实组 g0–g3 上 Haiku 并列率仅 4/24 且无锚点对上有编造证据("已成功
  采集木头"),Sonnet-low 并列率 17/24 且全部非并列判决锚定客观事件;有锚点对上两模型
...
- **bc_vpt4 行为级验证 run 巡检(16:2x)**:run 在跑(g2 判官/采集间隙,GPU 低谷正常),
  g0/g1 入档全绿(fallback/judge_call_fail=0)。行为信号:g0 判官报告 r2 出现挖掘进度条
  (canonical 32 条从未有);攻击占比 g0 均值 0.17(基线 0.09,两条超旧上限 0.14);
  g1 同 seed 对照两条挖穿泥土(dirt@1859/1246),前进占比 0.21-0.27 vs 基线 0.37-0.46
  ——"钉住"模式跨组复现。oak_log 仍 0。轨迹页已更新(artifact bcvpt4-g0)。
  下载器已重启换新转换代码(新段自带标签),池 344 段,磁盘 50%。
- **验证 run 巡检(17:2x)**:g5 采集中(GPU 100%),g2-g4 入档全绿。行为对照累计(五组):
  驻停模式 5/5 复现(前进占比一路降至 g4 的 0.08-0.13,基线 0.36-0.51);攻击强化 3/5 组
...
- **GRPO 环境(本机)全就绪**:CraftGround 首次构建过(JDK21 完整版坑入 knowledge/README.md);
...
- **慢塔 A/B 定案(§10.1 (d)(e) 缺口补测,→§10.2)**:同 32 题逐字节同输入,
  ...
  坑:Qwen FP8 在 sm_120 要 VLLM_USE_DEEP_GEMM=0(入 knowledge/README.md)。
