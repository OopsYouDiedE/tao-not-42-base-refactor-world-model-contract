"""从 BC 视觉 LoRA 生成带逐 token 行为概率的 Lumine policy rollout。"""
...
from lumine import decode_lumine_action
...
def generate_policy_rollouts(
    ...
    *,
    ...
    for _ in range(count):
        ...
        action = decode_lumine_action(text, expected_chunks=None)
        if len(action.chunks) < 8:
            raise RuntimeError(f"模型输出只有 {len(action.chunks)} tick，低于合同要求的 8 tick")
        generations.append(
            PolicyGeneration(
                action_text=text,
