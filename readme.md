...
如果准备先释放 GPU，可以在不带 CUDA 的实例上只下载数据。必需的完整 action 与
...
long_horizon 执行即可预取完整主课程。三个阶段全部 image 约 642.5 GB，加全量 action
约 30.5 GB，必需原始数据共约 673.0 GB；考虑 checkpoint、文件系统余量和缓存，建议
预留 720–750 GB。可选 meta_info 另占约 75.3 GB，启用后建议预留约 800 GB。下载特定
...
- 每次保留该阶段全部 action 和一个正在训练的 image LMDB；meta_info 默认不下载；
...
需要排查单个分片时，可手工下载当前阶段的全部动作 LMDB 和一个图像 LMDB：
...
单个图像分片通常为几十 GB。meta_info 默认既不下载也不参与 episode 交集；如需逐帧
GUI 状态和绝对光标坐标作诊断，可在下载命令中使用 `--modalities meta_info`，并在
训练入口增加 `--include-metadata-targets`。这些字段当前只随 batch 返回，不计入 loss，
也绝不直接送入策略。MineStudio 和 CraftGround 都把
...
训练入口默认直接读取 MineStudio v1.1 的 `image + action` LMDB，可选读取 meta_info，
不安装
