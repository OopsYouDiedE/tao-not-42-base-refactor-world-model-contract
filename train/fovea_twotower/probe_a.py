...
通过判据(step1 §5):FULL 的 attack AUC ≥ FRAME + 0.05。
...
def collect(model, dino, dl, n_max, dev):
    """→ feats_full [N,d], feats_frame [N,d], y_attack [N], mouse_mag [N]"""
    F_full, F_frame, ya, mm = [], [], [], []
    for batch in dl:
    ...
    return cat(F_full), cat(F_frame), cat(ya), cat(mm)
...
def main():
    ...
    Xf_tr, Xr_tr, ya_tr, mm_tr = collect(model, dino, mk("train"), args.n, dev)
    Xf_te, Xr_te, ya_te, mm_te = collect(model, dino, mk("holdout"), args.n, dev)
    ...
    res["verdict"] = ("PASS" if d >= 0.05 else "FAIL") + f" (Δattack={d:+.4f})"
