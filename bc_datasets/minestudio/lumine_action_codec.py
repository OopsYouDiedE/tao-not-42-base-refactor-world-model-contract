"""Lumine run-length 动作格式的编解码（arXiv 2511.08892 的动作表示）。
...
对外接口：
    ...
    LumineWindowAction — 一个感知窗口的完整动作（鼠标增量 + 若干 chunk）。
...
格式约定（照搬 Lumine，帧率换成 Minecraft 的 20Hz）：
感知端每个窗口做一次前向，窗口内按电机步切成若干 chunk，每个 chunk 只列出**该 chunk
按住的键**。时长靠 run-length 表达——键在相邻 chunk 连续出现即保持按下、不重复按下；
某 chunk 缺席即在该 chunk 松开。格式里没有时长数值，也没有按下 / 松开事件 token，
以此规避"按 3 秒还是 2 秒"这类脆弱的时长回归。

串形状::

    <|action_start|>ΔX ΔY ΔZ ; K K ; ; K <|action_end|>

``ΔX ΔY`` 是整数鼠标像素增量（钳到 ±1000），``ΔZ`` 是滚轮档位（钳到 ±5），随后每个
``;`` 分隔一个 chunk，chunk 内以空格分隔键名，空 chunk 表示该步全部松开。
"""
...
class LumineActionChunk:
    ...
    ----------
    keys : tuple of str
        该 chunk 按住的键名，按 ``MINECRAFT_KEYMAP`` 的声明顺序排列。空元组表示全松开。
    """
...
class LumineWindowAction:
    ...
    Attributes
    ----------
    mouse_delta_x, mouse_delta_y : int
        窗口内累计的鼠标像素增量，已钳到 ``±MOUSE_DELTA_LIMIT``。x 对应 yaw，y 对应 pitch。
    scroll_delta : int
        滚轮档位增量，已钳到 ``±SCROLL_LIMIT``。
    ...
    def to_text(self) -> str:
        """序列化为 Lumine 动作串。"""
        header = f"{self.mouse_delta_x} {self.mouse_delta_y} {self.scroll_delta}"
        body = f" {CHUNK_SEPARATOR} ".join(" ".join(chunk.keys) for chunk in self.chunks)
        return f"{ACTION_START}{header} {CHUNK_SEPARATOR} {body}{ACTION_END}"
...
def encode_lumine_action(
    ...
    -------
    LumineWindowAction
        鼠标增量为窗口内累计值（Lumine 按 delta 在窗口内平滑铺开执行），
        chunk 列表长度为 ``T // frames_per_chunk``。
    ...
    # camera 列序是 [pitch, yaw]：pitch 抬头低头对应鼠标 Y，yaw 左右转对应鼠标 X。
    total_pitch, total_yaw = camera.sum(axis=0)
    # 分母是模块常量，恒 > 0，不需要 clamp；这里显式挡住调用方传 0。
...
def decode_lumine_action(
    ...
    for index in range(3):
        try:
            numbers.append(int(float(header[index])))
        except (IndexError, ValueError):
            numbers.append(0)
    delta_x = _clamp(numbers[0], MOUSE_DELTA_LIMIT)
    delta_y = _clamp(numbers[1], MOUSE_DELTA_LIMIT)
    scroll = _clamp(numbers[2], SCROLL_LIMIT)
    ...
    for segment in segments[1:]:
        ...
        chunks.append(LumineActionChunk(keys=tuple(seen)))
