...
```bash
python -m dataset.organization.generate_questions \
...
```bash
python -m dataset.review.questions \
...
```bash
python -m dataset.review.actions \
  --dataset-dir runs/datasets/minestudio-trajectory-candidates \
  --raw-dataset-dir runs/datasets/minestudio-data-10xx-v110 \
  --host 127.0.0.1 \
  --port 7861
...
```bash
python -m dataset.organization.pack_hdf5 \
...
from dataset.organization.load_hdf5 import load_hdf5_conversations
