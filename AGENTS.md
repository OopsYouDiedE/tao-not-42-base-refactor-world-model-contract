...
> 训练配方与当前待办以 `knowledge/README.md` §5 为准（SSOT，本节只管"把机器点亮"）。
...
两塔同口径合规/延迟对比结论见 `knowledge/README.md` §3。
...
- 说人话：陈述句直接给结论；少用比喻式词汇。既定项目术语（"苦涩的教训"、"快塔/慢塔"、
  "判官"、"暖启动"等）保留；"解药/翻车/死穴/救不了"一类修辞改为陈述
  （"对策是 X"、"该做法出错"、"该缺陷无法由 Y 修复"）。
- 少用转折从句和多层嵌套从句，一句话说一件事；长句拆成多个短陈述句。
- 使用书面语言，不用口语化表达。
