...
SETUP_COMMANDS = (
    ...
    "summon minecraft:cow 7 64 7 {NoAI:1b,PersistenceRequired:1b,Health:7.0f,Tags:[\"memory_snapshot_probe\"]}",
)
...
ASSERTION_COMMANDS = (
    ...
    'execute if data block 3 64 1 {Items:[{Slot:0b,id:"minecraft:diamond",count:7},{Slot:1b,id:"minecraft:apple",count:5}]} run say MEMORY_CHEST_OK',
    'execute if data block 4 64 1 {Items:[{Slot:0b,id:"minecraft:raw_iron",count:3},{Slot:1b,id:"minecraft:coal",count:2}]} run say MEMORY_FURNACE_ITEMS_OK',
...
def _build_environment(runtime: Path) -> Any:
...
def _step_commands(environment: Any, commands: tuple[str, ...], ticks: int) -> Any:
    from craftground.environment.action_space import no_op_v2

    environment.add_commands(list(commands))
    observation = None
...
def verify(runtime: Path) -> dict[str, Any]:
    environment = _build_environment(runtime)
    try:
        environment.reset(options={"fast_reset": False})
        _step_commands(environment, SETUP_COMMANDS, ticks=20)
        ...
        _step_commands(environment, MUTATION_COMMANDS, ticks=10)
        reset_timings = coordinator.reset_all(snapshot)
        restored_observation = _step_commands(environment, (), ticks=8)
        assertion_messages: list[str] = []
        for assertion in ASSERTION_COMMANDS:
            asserted_observation = _step_commands(environment, (assertion,), ticks=2)
