...
- 数据:OpenAI VPT 承包商录像(all_6xx/7xx 索引,公开 blob),训练池滚动增长,收尾时
  58 clips / 33.7 万 ticks(≈4.7 小时,下载器仍在攒);holdout 独立 2 clips
  (GUI 帧两侧一致剔除后 10733 ticks)。
...
- 吞吐:8.6–10.3k ticks/s @ GPU 90%+(帧堆叠必须 GPU 侧索引,CPU 展开会饿死 GPU,
  见 commit 10156e4)。

| 口径(holdout,GUI 剔除) | 多数类基线 | BC 最优(run3@600) |
|---|---|---|
| cam top-1 acc(全 tick) | 0.8225(恒零 bin) | **0.8300** |
| cam acc(非零相机 tick,占 0.1775) | 0 | **0.1617**(中程峰 0.20–0.22) |
| 键 F1(有支持键均值) | 0(全不按) | **0.6508** |
| holdout ce+bce | — | **0.6350** |
...
三次 run 的训练动力学(数据小是主约束):run1 恒定 lr 3e-4 在 step 600 后 holdout 单调
恶化(0.66→0.93);run2 加 cosine 衰减到 0.6597@1200;run3 数据池扩到 58 clips 后
0.6350@600,但 15000 步末端仍过拟合到 2.77——4.8M 参数在小时级数据上必然记忆,
**最优点恒在 600–1200 步,数据量是纯规模杠杆**(公开 blob 有数千段可继续加)。
...
checkpoint:`runs/checkpoints/bc_vpt/best.pt`(gitignored,产物不入库);
