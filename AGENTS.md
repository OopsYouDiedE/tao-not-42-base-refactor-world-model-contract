...
- `PixelTower` 与 `SpatiotemporalFastTower` 两代快塔。
...
| `datasets/vpt/` | VPT `mp4 + jsonl` 读取和 VPT 原始动作编码 |
...
| `train/minecraft/` | Minecraft 行为克隆优化循环与训练装配 |
...
- 行为克隆训练属于 `train/minecraft/`，CraftGround 执行动作契约属于
