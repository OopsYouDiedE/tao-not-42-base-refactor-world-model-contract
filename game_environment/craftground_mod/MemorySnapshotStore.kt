...
object MemorySnapshotStore {
    ...
    fun handle(command: String, client: MinecraftClient): Boolean {
        if (!command.startsWith("memorysnapshot ")) {
            return false
        }
        ...
        when (operation) {
            "save" -> {
                ...
                server.execute {
                    ...
                    snapshots[snapshotId] = Snapshot(template, minimum)
                }
            }
            "load" -> {
                ...
                server.execute {
                    val world = server.overworld
                    val snapshot = requireNotNull(snapshots[snapshotId]) {
                        "内存快照不存在：$snapshotId"
                    }
