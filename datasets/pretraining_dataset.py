...
Lumine 原文的 6×33ms 是《原神》的 30Hz 电机步，这里按 Minecraft tick 换算，不照抄。
"""
...
from datasets.episode_split import HoldoutLevel, SplitResult, build_split
from datasets.minestudio_data.load import TrajectoryReader
from datasets.action_codec import (
    MINECRAFT_KEYMAP,
    encode_lumine_action,
)
from datasets.variable_action_contract import validate_action_image_alignment
...
class WindowLayout:
    ...
    def __post_init__(self) -> None:
        ...
        if self.window_frames % self.frames_per_chunk != 0:
            raise ValueError(
                f"window_frames {self.window_frames} 必须能被 frames_per_chunk "
                f"{self.frames_per_chunk} 整除",
            )
    ...
    def chunks_per_window(self) -> int:
        """一个感知窗口产出的电机 chunk 数。"""
        return self.window_frames // self.frames_per_chunk
...
class PretrainingSample:
    ...
    action_text : str
        本窗口的 Lumine 动作串（监督目标）。
    previous_action_text : str
        上一个窗口的 Lumine 动作串；无前序窗口时为空串。
    """
...
def _observation_frame_indices(start_frame: int, layout: WindowLayout) -> list[int]:
    ...
    """
    indices = [
        start_frame - offset * layout.window_frames
...
def _iterate_episode_samples(
    ...
    for start in range(first_start, total_frames - layout.window_frames + 1, layout.stride_frames):
        ...
        action = encode_lumine_action(window, frames_per_chunk=layout.frames_per_chunk)
        ...
        if previous_start >= 0:
            previous_window = action_reader.read_frames(
                episode, previous_start, layout.window_frames,
            )
            previous_action_text = encode_lumine_action(
                previous_window, frames_per_chunk=layout.frames_per_chunk,
            ).to_text()
        ...
        if has_image and frames_directory is not None:
            ...
            aligned_images = reader.readers["image"].read_frames(
                episode, start, layout.window_frames,
            )
...
def build_pretraining_dataset(
    ...
    info: dict[str, Any] = {
        ...
        "frames_per_chunk": resolved.frames_per_chunk,
        "chunks_per_window": resolved.chunks_per_window,
...
def main() -> None:
    ...
    layout = WindowLayout(
        ...
        frames_per_chunk=arguments.frames_per_chunk,
