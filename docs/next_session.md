...
**▶ 结果(07-08 深夜,commit ee55896)**:训练侧 chunk_k 已落地——TrackNavTower 头出
k 步、块内 CE 均匀权重(不引折扣)、留出评首步 cam_acc+key_f1;**k=1 与旧口径逐字兼容**
(v17 ckpt 直接加载+推理验过,保护 R-A rollout 路径),k=1/4/8 训练 smoke 无形状错。
v17 配方复位:data=trackcmd_v13{,_dagger},d512/layers7/seq64/prev_dropout0.5/switch_os0.5。
**待跑**:3 臂正式训练(--total_steps 3000)+ StudentPolicy 分块开环执行 +
闭环 eval_track_cmd 挖掘持续段(attack≥23tick 段占比)判据 → 定 k。
...
| 设定 | 由哪轮定 | 值(待填) |
|---|---|---|
| 快塔 chunk k / 块内权重 | R-B | 代码就绪(块内均匀权重);k 值待 3 臂闭环定 |
| 快塔规模×示范量预算 | R-G | |
