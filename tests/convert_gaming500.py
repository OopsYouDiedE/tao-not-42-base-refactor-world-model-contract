...
def parse_args():
    ...
    p.add_argument("--crop-stream", action="store_true",
                   help="每段额外输出 {stem}_crop{N}.mp4:从 1080p 原生像素裁运动能量最大的 "
                        "N×N 窗口(不缩放,保留高频细节),专喂 tokenizer——学习式注意 v0 的"
                        "转换期实现(knowledge/design_learned_attention.md §3);无 jsonl 配对,"
                        "vpt_dataset._pair_list 自动忽略,不进动力学训练")
    p.add_argument("--crop-size", type=int, default=256, help="裁剪流窗口边长(1080p 原生像素)")
    p.add_argument("--seed", type=int, default=0)
    return p.parse_args()


def probe_nvenc():
    """探测 NVDEC/NVENC 可用性(Colab 换机后 GPU 型号可能变,开机自检)。"""
    r = subprocess.run(
        ["ffmpeg", "-y", "-loglevel", "error", "-f", "lavfi", "-i", "color=black:size=256x256:rate=30",
         "-frames:v", "3", "-c:v", "h264_nvenc", "-f", "null", "-"],
        capture_output=True)
    ok = r.returncode == 0
    print(f"🎛️  NVENC {'可用——解码 NVDEC/编码 NVENC 走 GPU 引擎(不占 CPU/SM)' if ok else '不可用,回落 libx264 CPU 路径'}",
          flush=True)
    return ok
...
def cut_video(src, dst, start_f, n_frames, fps, vf, use_gpu):
    """帧精确切段 + 任意 -vf 滤镜重编码(-ss 放 -i 后逐帧精确)。

    use_gpu:NVDEC 解码(帧回系统内存过 CPU 滤镜)+ NVENC 编码——解/编码引擎独立于
    SM 与 CPU 核,实测 1080p 全链 ~20-25× 实时且可与训练并行;不可用时回落 libx264。
    NVENC cq 30 ≈ x264 crf 23 体积(NVENC 压缩效率略低,已实测标定)。
    """
    dec = ["-hwaccel", "cuda"] if use_gpu else []
    enc = ["-c:v", "h264_nvenc", "-preset", "p4", "-cq", "30"] if use_gpu \
        else ["-c:v", "libx264", "-preset", "veryfast", "-crf", "23"]
    cmd = (["ffmpeg", "-y", "-loglevel", "error"] + dec + ["-i", src,
           "-ss", f"{start_f / fps:.3f}", "-frames:v", str(n_frames),
           "-vf", vf, "-r", str(fps)] + enc + ["-an", dst])
    subprocess.run(cmd, check=True)


def motion_crop_window(mp4, start_f, end_f, crop_size, crop_bottom, n_probe=16):
    """采样 n_probe 帧算帧差能量图,返回能量最大的 crop_size² 窗口左上角 (x, y)。

    学习式注意 v0 的转换期实现:saliency 头就绪前,用运动能量(帧差)近似
    "值得高清看的区域"。能量图在 1/6 分辨率(320×180)上算,积分图找最大窗口。
    """
    cap = cv2.VideoCapture(mp4)
    W = cap.get(cv2.CAP_PROP_FRAME_WIDTH) or 1920
    H = cap.get(cv2.CAP_PROP_FRAME_HEIGHT) or 1080
    sw, sh = 320, 180
    energy, prev = np.zeros((sh, sw), np.float64), None
    for fi in np.linspace(start_f, end_f - 1, n_probe).astype(int):
        cap.set(cv2.CAP_PROP_POS_FRAMES, int(fi))
        ret, f = cap.read()
        if not ret:
            continue
        g = cv2.cvtColor(cv2.resize(f, (sw, sh), interpolation=cv2.INTER_AREA),
                         cv2.COLOR_BGR2GRAY).astype(np.float64)
        if prev is not None:
            energy += np.abs(g - prev)
        prev = g
    cap.release()
    # 排除底部任务栏区域;窗口在能量积分图上滑动取最大
    k = max(4, int(round(crop_size * sw / W)))                  # 窗口边长(能量图尺度)
    y_max = int(sh * (1 - crop_bottom)) - k
    ii = cv2.integral(energy)                                    # [sh+1, sw+1]
    best, bx, by = -1.0, 0, 0
    for y in range(0, max(1, y_max)):
        row = ii[y + k, k:sw + 1] - ii[y + k, :sw + 1 - k] \
            - ii[y, k:sw + 1] + ii[y, :sw + 1 - k]
        x = int(row.argmax())
        if row[x] > best:
            best, bx, by = float(row[x]), x, y
    X = min(int(bx * W / sw), int(W) - crop_size)
    Y = min(int(by * H / sh), int(H * (1 - crop_bottom)) - crop_size)
...
def main():
    ...
    for si, sess in enumerate(sessions):
        ...
        if glob.glob(os.path.join(args.out, f"{sid}_*.mp4")):
            print(f"[{si + 1}/{len(sessions)}] {sid} 已存在,跳过(断点续转)", flush=True)
            continue
        ...
        vf_full = (f"crop=iw:ih*{1 - args.crop_bottom:.3f}:0:0,scale=-2:{args.scale_h}"
                   if args.crop_bottom > 0 else f"scale=-2:{args.scale_h}")
        for gi, (s, e) in enumerate(segs):
            stem = os.path.join(args.out, f"{sid}_{gi:02d}")
            try:
                cut_video(mp4, stem + ".mp4", s, e - s, 30, vf_full, use_gpu)
                if args.crop_stream:
                    cx, cy = motion_crop_window(mp4, s, e, args.crop_size, args.crop_bottom)
                    cut_video(mp4, f"{stem}_crop{args.crop_size}.mp4", s, e - s, 30,
                              f"crop={args.crop_size}:{args.crop_size}:{cx}:{cy}", use_gpu)
