...
| L2.5 第三方 | `net/dreamer/` | 原样 vendored 的 DreamerV3（NM512/dreamerv3-torch，MIT；见其 `NOTICE`/`LICENSE`）。配套底层算子按分层拆到 `blocks/{distributions,sequence,conv,dynamics(GRUCell)}` 与 `utils/nn`（init/训练胶水），均标"原样照抄"。**类体逐字 1:1，不要为贴 house style 改 vendored 文件体**；升级方式是重拉上游覆盖（见 knowledge/dreamer.md） | 我们改写的逻辑 |
...
```
- `net/` **不得** import `train/`（契约数值如 `N_CAMERA_BINS` 在 net 内独立声明，
  ...
  `ModelConfig` 再传入模型；领域常量（act_dim / n_cam_bins）由 `train/<game>/` 持有并断言一致。
...
- `train/minecraft/vpt_dataset.py` 等文件头仍保留成段设计叙事（历史遗留）；按 §2.3 应逐步下沉
  到 `knowledge/`，代码留指针。
- `knowledge/mental_world.md` 描述的是**设计愿景**（含 fovea/gaze 等尚未落地于当前活模型的部件），
