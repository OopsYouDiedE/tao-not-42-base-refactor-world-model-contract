...
from net.vpt_lib.minecraft_util import store_args

# 本仓库 vendored 版:已去除 `import attr` 与 `import minerl...`(teacher 必须 minerl-free)。
# CameraQuantizer 原用 @attr.s,改写为普通 __init__;item_embed_*(依赖 minerl 的物品表)
# 仅 non-human 动作空间用到,人类按键策略从不触及,已随 mc 导入一并移除。
...
class CameraQuantizer:
    ...
    def __init__(self, camera_maxval, camera_binsize,
                 quantization_scheme=QuantizationScheme.LINEAR, mu=5):
