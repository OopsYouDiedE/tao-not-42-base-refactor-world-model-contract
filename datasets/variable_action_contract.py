...
def action_ticks(actions: dict[str, np.ndarray]) -> list[dict[str, Any]]:
    ...
    names = {
        ...
        "attack": "MouseLeft", "use": "MouseRight",
    }
    for index, (pitch, yaw) in enumerate(camera):
        keys = tuple(names[field] for field in key_fields if field in actions and bool(actions[field][index]))
...
def expand_action_segments(segments: list[ActionSegment]) -> list[dict[str, Any]]:
    ...
    for segment in segments:
        ticks.extend(
            {"keys": segment.keys, "mouse": segment.mouse}
            for _ in range(segment.duration_ticks)
        )
    return ticks
    ...
    *,
    ...
    return {
        ...
        "irregular_mouse_frames": sum(
            segment.duration_ticks == 1 and segment.mouse != (0, 0)
            for segment in segments
        ),
    }
