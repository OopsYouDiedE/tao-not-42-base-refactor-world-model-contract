...
from tools.trajectory_action_review import ActionReviewStore, optimize_action_sequence
...
def dataset(root: Path) -> None:
    question = {"id": "q", "task_type": "image_sequence_to_action", "images": ["a", "b"], "source": {"image_frames": [10, 16], "episode": "e"}, "target_interval": [10, 16]}
...
def test_gameplay_compression_preserves_held_duration_and_merges_mouse() -> None:
    action = "<|action_start|> ; Mouse 1 0 W ; Mouse 2 0 W ; W ; Mouse 4 1 W ; Mouse 5 2 W <|action_end|>"
...
def test_save_rejects_wrong_block_count(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="需要 1 个动作块"):
        store.save("q", json.dumps(["<|action_start|> ; W <|action_end|>"] * 2), "", "approve", "理由")
