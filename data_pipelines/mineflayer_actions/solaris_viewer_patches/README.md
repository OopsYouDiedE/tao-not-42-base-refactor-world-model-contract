...
> solaris 引擎本身在独立仓库 mc-agents（非本项目）。这些补丁打在它 `npm install` 出来的
> `node_modules/prismarine-viewer-colalab` 副本上，因此以 patch 形式固化在这里，而非改引擎源码。
...
# 例:bash apply.sh ~/mc-test/solaris-engine/node_modules/prismarine-viewer-colalab
```
...
验收样本见同层 `../sample/action_boundaries/`（真实草地画面 + 动作起止截图）。
