...
4. **GRPO 精修**：前置是有慢塔的机器（5090/Blackwell）或换塔（备选见设计文档 §10）。
   ...
   **死亡即截断 episode，证据文本写明「第 N 步死亡」**。附带排序失真证据：该 run
   ...
   ▶ **本地 RM 升级阶梯（用户认可，2026-07-10）**：判官 A/B 实测显示有效判决全部
   锚定客观事件（里程碑/死亡/空白帧），软性"意图质量"判断是幻觉源——RM 的真实任务
   是视觉事件判别+比较，不是语言推理。阶梯：(一) 攒干净数据（pairwise_v2 协议 +
   客观锚点过滤；tie 对以 margin=0 入训练集），启动门槛千级有效对（现约百级、71% tie）；
   (二) 第一级=冻结 DINO patch（复用 v2 前端）+ 小打分头，成对偏好损失，分钟级训练、
   毫秒级推理，成立即把"每组 1 次 API 排序"换成无限次本地打分；(三) 第一级留出对
   准确率证伪后才升第二级=慢塔 LoRA 判官（r16/qkvo 配方已裁决；vLLM base+adapter
   同服、规划器判官共权重；注意 FP8 基座需 BF16/NF4 训练副本、自评分布窄化风险）。
   不许跳级（§6 纪律）。
   ▶ **GRPO 上量配置候选（信号存在性证实后）**：组数优先于 tick（§4 信息公式）——
   形状约 50–100 组 × 6–8 条 × 800 tick + 有树 seed 筛选（树旁出生时 800 tick 足够
   出第一个里程碑事件）；前置=hindsight relabel BC 让 goal 通道可听（8×4×2000 run
   实证：慢塔指令正确但快塔听不见，FiLM 在 BC 期从未被训练）。
...
5. **里程碑接力起点课程（用户提出 2026-07-11，触发条件=首木 GRPO 起效后）**：
   ...
   此课程只服务 GRPO 精修的纵深扩展。
   ▶ **精确中途态（用户追问 2026-07-11）**：give/tp 复现不了地形改动/掉落物等过程态。
   既定机制下的等价物=**seed+动作序列重放**（冻结时相+关生物生成 ⇒ 若引擎确定性成立，
   重放前 T 步即精确重建一切状态，成本≈一次 env 重启；且解锁 GRPO 中途分叉采样——
   组内共享真实前缀、从相同中途态分岔，信用分配归因后段）。**前置探针（预登记）**：
