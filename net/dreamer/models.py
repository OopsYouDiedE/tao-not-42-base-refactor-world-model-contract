"""DreamerV3 的顶层模型:WorldModel(编码+RSSM+解码+奖励/继续头)与 ImagBehavior(想象中的 actor-critic)。

对外接口:RewardEMA、WorldModel、ImagBehavior。

来源:原样照抄 NM512/dreamerv3-torch 的 `models.py`(MIT,见 net/dreamer/NOTICE)。
仅顶部 import 改为指向本仓拆分后的位置(networks 同包、tools→net.dreamer._compat);
以下类体(含 _train/_imagine/损失)保持 1:1 原样。
"""
import copy
import torch
from torch import nn

from net.dreamer import networks
