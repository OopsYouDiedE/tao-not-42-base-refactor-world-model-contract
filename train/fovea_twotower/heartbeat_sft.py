...
def build_prompt(window):
    """状态行窗口 → 决策 prompt。换目标的<下一个>由 goal 字段(里程碑达成即前移)
    可观测,无需外挂链;实测加链+规则文本反而过触发重规划(N=20 0.937→0.662),故从简。"""
    return ("你是 Minecraft 智能体的慢脑心跳决策器。下面是最近若干次心跳的状态行,"
            "请对最新一行给出微决策,只输出以下之一:继续 / 重规划 / 换目标:<物品>。\n\n"
            f"{window}\n\n决策:")
