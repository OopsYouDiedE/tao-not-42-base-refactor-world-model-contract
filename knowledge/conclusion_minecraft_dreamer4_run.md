...
2. **C（|Δz|² 损失加权）赢在单步像素、输在动力学**：gen−persist 唯一非负（+0.01dB），
   genmean 四组最高（23.40dB），但 EV 恒负、开环最差——损失加权把单步预测推向大变化
...
bf16 autocast（三精度 60 步对照损失逐位一致，bf16 较 fp32 +26% 吞吐,见 git 历史 dreamer.md §2.6），
...
64px 观测，评估口径见 git 历史 `knowledge/dreamer.md` §2。
...
## 6. 开环优势指标的结构性解读（用户问"为何总输给复读"，2026-07-02）

roll_adv 恒负的两个结构性原因（非模型无能的证据）：
1. **重建税**：生成路径过 tokenizer 有损瓶颈,动力学再完美天花板也是 psnr_recon;
   persistence 复读无损真实像素、不交税。真实可赢空间 = recon − persist,
   动力学误差从这点预算里扣。
2. **静止像素稀释**：30Hz 下 8 帧≈0.27s,天空/HUD/远景几乎不动,persistence 在
   静止像素上数学上不可战胜;PSNR 按像素平均,模型在变化区域的赢分被摊没。
   指标结构性偏袒复读——这正是引入 EV(Δz)(persistence 恒 0)的原因。

转正条件与对应动作：高运动片段(建议 evaluate 增加按运动分位数条件化的优势口径,
挖矿恰发生在高运动时刻)、更长推演视野(persistence 线性崩,16/32 步差距显形)、
recon 天花板抬高(128²/S=64 跑的目的之一)。领域共识:高帧率短视野 raw PSNR
打不过复读是常态,delta 类/感知类指标才是世界模型能力的干净口径。

## 7. 128² 容量基线终评(2026-07-02,runs/mc_d4_b128 @10k 步收割)

**设定**:B 配方(motion_sample=4)不变,唯一大变量 = img_size 64→128(token 网格
4×4=16 → 8×8=64,隐码容量×4);646M 参数(~600M 为解码器平铺 Linear,已知浪费),
batch 16,10k 步 × 256 帧/步 = 256 万帧——与 64² B@5k(batch 32)**同帧预算**,
是"容量解闸"假设的干净对照。收割点由 15k 提前至 10k(LR 无退火,中途收割零损失)。

**结果(vs 64² B@5k,同帧预算)**:

| 口径 | 64² B | 128² B | 判定 |
|---|---|---|---|
| gen−persist(单步) | −0.12dB | **+1.20dB** | 首次大幅转正 |
| EV(Δz) | +0.037 | **+0.229** | ×6 |
| 8 步开环优势 | −0.31dB | **+0.74dB** | 首次转正 |
| recon 上限 | 26.45 | 28.21 | 天花板同步抬升 |
| IG | −0.308 | −0.215 | 仍负,遗留诊断项 |

best.pt 落在 step 9500(gen 23.02 / EV +0.238 / 开环 +0.88dB)。轨迹上:EV 于
3500 步转正后单调走高;开环优势从 @4000 的 −1.96dB 一路收窄,@6000 首次触正,
@9500-10000 稳定在 +0.7~0.9dB——不是抖动,是趋势。

**判定:三闸分析中"闸 2(隐码瓶颈)是当前主束缚"的假设成立。** §6 预言的
"EV 是干净口径、容量抬升会同时救开环"均兑现:模型第一次在所有推演口径上
超过"什么都不做"的朴素复读。代价:计算量/帧 ~7×(340 vs ~2300 帧/s)。

**下一步含义**:(1) 单变量消融的下一轮(hard_weight、gaming500 数据)应以本跑为
新基线;(2) 解码器平铺 Linear 的 600M 浪费在 S=64 下已不可忽视,升级为卷积/反卷积
头的收益排序上移;(3) IG 恒负跨 64²/128² 复现,与容量无关,需专项诊断(v1 学习式
挑帧的前置);(4) 目视验收见 runs/viz/b128_10k.mp4(对比 1.5k 步版本)。

## 8. hard_weight=1.0(OHEM v0)消融:负结果,提前止损(2026-07-02)

128² 基线口径唯一变量 `--hard_weight 1.0`(误差比例重加权,[0.25,4] 有界)。
@2500/3000/3500 三连评全口径大幅落后基线且无收敛迹象:@3500 gen=20.97(基线 22.26)、
EV=-0.323(基线 +0.087)、开环 -4.10dB(基线 -1.15)。3500 步即终止,省 ~1.4h GPU。

**解读**:按自身流误差加权 = 把权重压向高运动/难转移,早期模型在难样本上梯度
方差本就大,放大 16 倍(0.25→4)直接破坏了流匹配的收敛;开环退化最大(-4.1dB)
说明误差复利被加剧。**若重试,方案:hard_weight≤0.3 + 前 2k 步禁用(warmup 后启用)**;
优先级降至数据/架构杠杆之后。

## 9. 数据变量实验启动:gaming500 生存 MC(runs/mc_d4_b128_g500)

- 动作数据验收:31 段 84.7 万帧,50.2% 帧带按键,键位词表与 vpt_action 精确一致,
  分布健康(w 34%>space>attack>use>d/a/s);|dx| p95=31 ⇒ camera_scale=32,
  恰与 findcave 同值,鼠标捕获坐标疑虑解除。
- 配方与 128² 基线完全一致,唯一变量 = 数据源(findcave 2.7h 定向任务 →
  gaming500 生存/教程 ~16h 多样玩法),10k 步同帧预算对照。

### §9 补:g500 数据变量实验中止记录(8.2k/10k 步,会话收尾主动停)

@8000:gen=21.60 persist=23.51(-1.91dB)EV(Δz)=-0.219 IG=-0.138 开环 -1.06dB。
**初步信号(未终评,仅供参考):同配方在 gaming500 生存数据上三口径全面差于
findcave 基线**(后者 @8000 约 EV+0.21/开环转正)。两个候选解释待下轮分辨:
(1) 数据更"静"(persist 基线高 1.7dB,菜单/教程画面多),EV 为负说明不只是口径
漂移,模型对该分布的 Δz 确实没学好;(2) clip_max_frames=9000 随机窗口 + 31 段
的多样性低于 findcave 承包商数据。下轮建议:数据混合(findcave+g500)而非替换,
或 g500 侧加强 --match 过滤。运行工程结论:clip_max_frames 修复后 6w×4c 稳定
327 帧/s 无 OOM,RAM ~24GB。
